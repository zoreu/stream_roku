...
COM SUPORTE DLNA INTEGRADO (CORRIGIDO - PARSE JSON)
"""
...
def handle_request(client_socket, client_address, server_socket):
    ...
    try:
        ...
        if path_part == "/":
            response_data = {
                "message": "ONEPLAY PROXY - QPython LAN Edition with DLNA",
                "status": "running",
                "local_ip": LOCAL_IP,
                "port": PORT,
        ...
        # ===== ROTA: /app =====
            ...
            try:
                ...
                r_html = http_client.request('https://raw.githack.com/zoreu/stream_roku/main/home_dlna.html')
        ...
        # ===== DLNA API ROUTES =====
            ...
            print(f"✅ {len(devs)} encontrados\n")
            
            response_data = {
                'success': True,
                'devices': [{
                    'id': d['id'], 'name': d['name'], 'manufacturer': d['manufacturer'],
                    'model': d['model'], 'ip': d['ip'], 'control_url': d['control_url'],
                    'tv_type': d['tv_type']
                } for d in devs]
            }
            ...
            print(f"Body recebido: {body_data[:200]}...")
            ...
            try:
                ...
                if not url or not control_url:
                    print(f"❌ Parâmetros faltando: url={url}, control_url={control_url}")
                    client_socket.sendall(b"HTTP/1.1 400 Bad Request\r\n\r\nMissing url or control_url")
                    return
                
                print(f"📡 Transmitindo para: {device_name}")
                ...
                # IMPORTANTE: URL do proxy deve terminar com barra para LG WebOS
            ...
            try:
                ...
                if not action or not control_url:
                    client_socket.sendall(b"HTTP/1.1 400 Bad Request\r\n\r\n")
                    return
                
                print(f"Controle: {action} para {control_url}")
